...
@section('title', 'تعديل البنر الإعلاني')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold">تعديل البنر الإعلاني</h2>
        ...
        <form action="{{ route('banners.update', $banner) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                @if($banner->image_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($banner->image_path))
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $banner->image_path) }}" alt="بنر حالي" class="rounded" style="max-width: 200px; max-height: 120px; object-fit: contain; border: 1px solid #dee2e6;">
                        <div class="small text-muted mt-1">الصورة الحالية. اختر ملفاً جديداً لاستبدالها.</div>
                    </div>
                @endif
                ...
                <small class="text-muted">الحد الأقصى: 5MB. اترك الحقل فارغاً للإبقاء على الصورة الحالية.</small>
                @error('image')
            ...
            <div class="mb-3">
                ...
                <div class="banner-size-cards">
                    @foreach(\App\Models\Banner::sizeLabels() as $value => $label)
                        <label class="banner-size-card {{ old('size', $banner->size) === $value ? 'selected' : '' }}">
                            <input type="radio" name="size" value="{{ $value }}" class="d-none" {{ old('size', $banner->size) === $value ? 'checked' : '' }} required>
            ...
            <div class="mb-3">
                ...
                <div class="banner-placement-cards">
                    <label class="banner-placement-card {{ old('show_on_home', $banner->show_on_home) ? 'selected' : '' }}">
                        <input type="checkbox" class="d-none" name="show_on_home" value="1" {{ old('show_on_home', $banner->show_on_home) ? 'checked' : '' }}>
                        <i class="bi bi-house-door me-2"></i>
                        <span>الصفحة الرئيسية</span>
                    </label>
                    <label class="banner-placement-card {{ old('show_on_video_detail', $banner->show_on_video_detail) ? 'selected' : '' }}">
                        <input type="checkbox" class="d-none" name="show_on_video_detail" value="1" {{ old('show_on_video_detail', $banner->show_on_video_detail) ? 'checked' : '' }}>
                        <i class="bi bi-play-circle me-2"></i>
                        <span>صفحة تفاصيل الفيديو</span>
                    </label>
                    <label class="banner-placement-card {{ old('show_on_categories', $banner->show_on_categories) ? 'selected' : '' }}">
                        <input type="checkbox" class="d-none" name="show_on_categories" value="1" {{ old('show_on_categories', $banner->show_on_categories) ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="order" name="order" value="{{ old('order', $banner->order) }}" min="0" step="1">
            </div>
