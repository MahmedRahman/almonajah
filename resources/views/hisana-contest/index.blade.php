...
<div class="card">
    <div class="card-body">
        @if($entries->count() > 0)
            <form method="POST" action="{{ route('hisana-contest.admin.bulk-destroy') }}" id="bulkDeleteForm" onsubmit="return confirm('حذف المشاركات المحددة؟');">
                @csrf
                @method('DELETE')
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @foreach($entries as $entry)
                                <tr>
                                    <td>
                                        <input class="form-check-input entry-check" type="checkbox" name="ids[]" value="{{ $entry->id }}">
                                    </td>
                                    <td>{{ $entry->id }}</td>
                                    <td>{{ $entry->name ?: '—' }}</td>
                                    ...
                                    <td class="text-nowrap">
                                        <button type="button" class="btn btn-sm btn-primary" onclick="document.getElementById('resend-form-{{ $entry->id }}').submit()">
                                            <i class="bi bi-envelope me-1"></i>إرسال
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="if(confirm('حذف هذه المشاركة؟')) document.getElementById('delete-form-{{ $entry->id }}').submit()">
            ...
            @foreach($entries as $entry)
                <form id="resend-form-{{ $entry->id }}" action="{{ route('hisana-contest.admin.resend', $entry) }}" method="POST" class="d-none">@csrf</form>
                <form id="delete-form-{{ $entry->id }}" action="{{ route('hisana-contest.admin.destroy', $entry) }}" method="POST" class="d-none">@csrf @method('DELETE')</form>
            @endforeach
