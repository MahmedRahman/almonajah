...
<div class="home-layout">
    ...
    <div class="main-content-wrapper">
        <div class="container-main">
            <!-- Scholars Grid -->
            @if($scholars->count() > 0)
                <div class="video-grid" id="scholarsGrid">
                    @foreach($scholars as $scholar)
                    <a href="{{ route('public.scholar.show', $scholar) }}" class="video-card">
                        <div class="video-thumbnail">
                            ...
                            @else
                                <div style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #188781 0%, #1f9f97 100%);">
                                    <i class="bi bi-person-badge text-white" style="font-size: 3rem;"></i>
                                </div>
                            @endif
                            <span class="video-duration">
                                <i class="bi bi-play-circle me-1"></i>{{ $scholar->assets_count ?? 0 }} فيديو
                            </span>
                        </div>
                        ...
                        <div class="video-info">
                            <div class="video-info-header">
                                <div class="video-channel-avatar">
                                    @if($scholar->image_path)
                                        <img src="{{ asset('storage/' . $scholar->image_path) }}" alt="{{ $scholar->name }}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">
                                    @else
                                        <i class="bi bi-person-badge" style="font-size: 1.5rem; color: var(--primary-color);"></i>
                                    @endif
                                </div>
                                <div class="video-info-content">
                                    <h3 class="video-title">{{ \Illuminate\Support\Str::limit($scholar->name, 60) }}</h3>
                                    @if($scholar->description)
                                        <div class="video-meta">
                                            <span class="video-channel-name">{{ \Illuminate\Support\Str::limit($scholar->description, 50) }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
            ...
            @else
                <div class="empty-state">
                    <p>لا يوجد شيوخ متاحون حالياً</p>
                </div>
            @endif
        </div>
    </div>
