...
@section('title', $scholar->name . ' - المناجاة')
        ...
        <div class="container-main">
            ...
            <div class="playlist-header mb-4" style="margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border-color);">
                <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                    <a href="{{ route('public.scholars') }}" style="color: var(--text-secondary); text-decoration: none; font-size: 0.875rem; display: flex; align-items: center; gap: 0.5rem;">
                        <i class="bi bi-arrow-right"></i> رجوع إلى الشيوخ
                    </a>
                </div>
                <div style="display: flex; gap: 1.5rem; align-items: flex-start;">
                    <div class="scholar-thumbnail-large" style="flex-shrink: 0; width: 200px; height: 200px; border-radius: 50%; overflow: hidden; background: linear-gradient(135deg, #188781 0%, #1f9f97 100%);">
                    ...
                    <div class="scholar-info-header" style="flex: 1;">
            ...
            <!-- Videos Grid -->
            @if($assets->count() > 0)
                <div class="video-grid" id="scholarVideoGrid">
            ...
            @else
                <div class="empty-state">
                    <p>لا توجد فيديوهات لهذا الشيخ</p>
                </div>
            @endif
        </div>
    </div>
...
.video-duration { position: absolute; bottom: 0.5rem; right: 0.5rem; background-color: rgba(0, 0, 0, 0.8); color: white; padding: 0.25rem 0.5rem; border-radius: var(--radius-sm); font-size: 0.75rem; font-weight: 600; }
.video-info { padding: var(--spacing-sm); }
.video-info-header { display: flex; gap: 0.75rem; margin-top: 0.75rem; }
.video-channel-avatar { flex-shrink: 0; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; background-color: var(--bg-tertiary); border-radius: 50%; overflow: hidden; }
...
.video-channel-name, .video-category { display: block; }
...
@media (max-width: 768px) { .sidebar-menu { width: 240px; top: 56px; height: calc(100vh - 56px); } .sidebar-menu:not(.collapsed) { width: 240px; } .playlist-header { flex-direction: column; } .scholar-thumbnail-large { width: 120px !important; height: 120px !important; } }
</style>
@endpush
